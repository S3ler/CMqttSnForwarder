...
int initialize_udp_socket(uint16_t port) {
    ...
    //FEATURE make this via bind_address command
...
ssize_t send_udp_message(int socket_fd, const device_address *to, const uint8_t *data, uint16_t data_length) {
    ...
    int     flags      = 0;
    ssize_t send_bytes = sendto(socket_fd, data, data_length, flags, (const struct sockaddr *)&to_sockaddr, to_sockaddr_socklen);
    if (send_bytes == -1) {
        return -1;
    }
    return send_bytes;
...
int32_t receive_udp_message(int socket_fd, uint8_t *buffer, uint16_t buffer_max_length, device_address *from) {
    ...
    if ((read_bytes = recvfrom(socket_fd, buffer, buffer_max_length, MSG_PEEK, (struct sockaddr *)&recv_sockaddr, &recv_sockaddr_socklen)) <
        0) {
        return -1;
    }
    ...
    if (read_bytes > buffer_max_length) {
        ...
        if ((read_bytes = recvfrom(socket_fd, buffer, buffer_max_length, MSG_WAITALL, (struct sockaddr *)&recv_sockaddr,
                                   &recv_sockaddr_socklen)) < 0) {
            return -1;
        }
        return 0;
    }
    ...
    if ((read_bytes =
           recvfrom(socket_fd, buffer, buffer_max_length, MSG_WAITALL, (struct sockaddr *)&recv_sockaddr, &recv_sockaddr_socklen)) < 0) {
        return -1;
    }
