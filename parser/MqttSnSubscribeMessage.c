...
static int32_t parse_subscribe_header(ParsedMqttSnHeader *h, const uint8_t *data, uint16_t data_len, int32_t *parsed_bytes);
static int32_t parse_subscribe_flags(const uint8_t *src_pos, uint16_t src_length, int32_t *parsed_bytes, uint8_t *dup, int8_t *qos,
                                     uint8_t *topic_id_type, uint16_t length);
static int32_t parse_subscribe_msg_id(const uint8_t *src_pos, uint16_t src_length, int32_t *parsed_bytes, uint16_t *msg_id);
static int32_t parse_subscribe_predefined_topic(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint16_t *topic_id);
static int32_t parse_subscribe_short_topic_name(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint16_t *topic_id);
static int32_t parse_subscribe_topic_name(const uint8_t *src_pos, uint16_t src_len, int32_t *parse_bytes, char *topic_name,
...
int32_t parse_subscribe_byte(uint8_t *dup, int8_t *qos, uint8_t *topic_id_type, uint16_t *msg_id, uint16_t *topic_id, char *topic_name,
    ...
    if ((parsed_bytes = parse_subscribe_flags(data + parsed_bytes, data_len, &parsed_bytes, dup, qos, topic_id_type, h.length)) < 0) {
        return -1;
    }
    ...
    if ((*topic_id_type) == MQTT_SN_FLAG_TOPIC_ID_TYPE_PREDEFINED_TOPIC_ID) {
        ...
        if ((parsed_bytes = parse_subscribe_topic_name(data + parsed_bytes, data_len, &parsed_bytes, topic_name, topic_name_length,
...
int32_t get_subscribe_msg_len(uint16_t topic_name_length) {
    if (topic_name_length > UINT16_MAX - MQTT_SN_MESSAGE_SUBSCRIBE_WO_TOPICNAME_MAX_LENGTH) {
        return -1;
    }
    uint16_t total_length = MQTT_SN_MESSAGE_SUBSCRIBE_WO_TOPICNAME_MIN_LENGTH + topic_name_length;
    if (total_length > UINT8_MAX) {
        total_length = MQTT_SN_MESSAGE_SUBSCRIBE_WO_TOPICNAME_MAX_LENGTH + topic_name_length;
    }
    return total_length;
}
int32_t generate_subscribe_topic_name_header(uint8_t *dst, uint16_t dst_len, uint16_t topic_name_length, int32_t *used_bytes) {
    int32_t total_length = get_subscribe_msg_len(topic_name_length);
    if (total_length < 0 || dst_len < total_length) {
        return -1;
    }
    return generate_mqtt_sn_header(dst, dst_len, used_bytes, total_length, SUBSCRIBE);
}
int32_t generate_subscribe_topic_name(uint8_t *dst, uint16_t dst_len, uint8_t dup, int8_t qos, uint16_t msg_id, const char *topic_name,
                                      uint16_t topic_name_length) {
    int32_t used_bytes = 0;
    if (generate_subscribe_topic_name_header(dst, dst_len, topic_name_length, &used_bytes) < 0) {
        return -1;
    }
    if (generate_flags(dst + used_bytes, dst_len, dup, qos, MQTT_SN_FLAG_RETAIN_FALSE, MQTT_SN_FLAG_WILL_FALSE,
...
int32_t generate_subscribe_topic_id_header(uint8_t *dst, uint16_t dst_len, int32_t *used_bytes) {
    int32_t total_length = MQTT_SN_MESSAGE_SUBSCRIBE_W_TOPICID_LENGTH;
    if (total_length < 0 || dst_len < total_length) {
        return -1;
    }
    return generate_mqtt_sn_header(dst, dst_len, used_bytes, total_length, SUBSCRIBE);
}
int32_t generate_subscribe_topic_id(uint8_t *dst, uint16_t dst_len, uint8_t dup, int8_t qos, uint8_t topic_id_type, uint16_t msg_id,
                                    uint16_t topic_id) {
    int32_t used_bytes = 0;
    if (generate_subscribe_topic_id_header(dst, dst_len, &used_bytes) < 0) {
        return -1;
    }
    if (generate_flags(dst + used_bytes, dst_len, dup, qos, MQTT_SN_FLAG_RETAIN_FALSE, MQTT_SN_FLAG_WILL_FALSE,
...
int32_t generate_subscribe(uint8_t *dst, uint16_t dst_len, uint8_t dup, int8_t qos, uint8_t topic_id_type, uint16_t msg_id,
    ...
    if (topic_id_type == MQTT_SN_FLAG_TOPIC_ID_TYPE_TOPIC_NAME && topic_name != NULL && topic_name_length > 0) {
        return generate_subscribe_topic_name(dst, dst_len, dup, qos, msg_id, topic_name, topic_name_length);
        ...
        return generate_subscribe_topic_id(dst, dst_len, dup, qos, topic_id_type, msg_id, topic_id);
    }
    return -1;
}
...
int32_t parse_subscribe_flags(const uint8_t *src_pos, uint16_t src_length, int32_t *parsed_bytes, uint8_t *dup, int8_t *qos,
                              uint8_t *topic_id_type, uint16_t length) {
...
int32_t parse_subscribe_predefined_topic(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint16_t *topic_id) {
    return parse_mqtt_sn_uint16_byte(src_pos, src_len, parsed_bytes, topic_id);
}

int32_t parse_subscribe_topic_name(const uint8_t *src_pos, uint16_t src_len, int32_t *parse_bytes, char *topic_name,
                                   uint16_t *topic_name_length, uint16_t topic_name_max_length) {
    return parse_mqtt_sn_char_until_end_byte(src_pos, src_len, parse_bytes, topic_name, topic_name_length, topic_name_max_length);
}
int32_t parse_subscribe_short_topic_name(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint16_t *topic_id) {
    return parse_mqtt_sn_uint16_byte(src_pos, src_len, parsed_bytes, topic_id);
}
