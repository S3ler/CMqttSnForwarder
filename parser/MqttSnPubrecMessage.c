...
#include "MqttSnPubrecMessage.h"

static int32_t parse_pubrec_header(ParsedMqttSnHeader *h,
                                   const uint8_t *data,
                                   uint16_t data_len,
                                   int32_t *parsed_bytes);
static int32_t parse_pubrec_msg_id(const uint8_t *src_pos,
                                   uint16_t src_length,
                                   int32_t *parsed_bytes,
                                   uint16_t *msg_id);
...
int32_t parse_pubrec_bytes(uint16_t *msg_id, const uint8_t *data, uint16_t data_len) {
  ...
  if ((parsed_bytes = parse_pubrec_header(&h, data, data_len, &parsed_bytes) < 0)) {
    return -1;
  }
  if ((parsed_bytes = parse_pubrec_msg_id(data + parsed_bytes, data_len, &parsed_bytes, msg_id)) < 0) {
    return -1;
  }
  return parsed_bytes;
}
...
int32_t parse_pubrec_header(ParsedMqttSnHeader *h, const uint8_t *data, uint16_t data_len, int32_t *parsed_bytes) {
  ...
  if (h->length != MQTT_SN_MESSAGE_PUBREC_LENGTH) {
    // invalid message length for pubrec
    return -1;
  }
  return *parsed_bytes;
}
