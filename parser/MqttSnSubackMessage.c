...
static int32_t parse_suback_header(ParsedMqttSnHeader *h,
                                   const uint8_t *data,
                                   uint16_t data_len,
                                   int32_t *parsed_bytes);
static int32_t parse_suback_topic_id(const uint8_t *src_pos,
                                     uint16_t src_length,
                                     int32_t *parsed_bytes,
                                     uint16_t *topic_id);
static int32_t parse_suback_msg_id(const uint8_t *src_pos,
                                   uint16_t src_length,
                                   int32_t *parsed_bytes,
                                   uint16_t *msg_id);
static int32_t parse_suback_return_code_byte(const uint8_t *src_pos,
                                             uint16_t src_len,
                                             int32_t *parsed_bytes,
...
int32_t parse_suback_bytes(uint16_t *topic_id,
  ...
  if ((parsed_bytes = parse_suback_header(&h, data, data_len, &parsed_bytes) < 0)) {
    return -1;
  }
  if ((parsed_bytes = parse_suback_topic_id(data + parsed_bytes, data_len, &parsed_bytes, topic_id)) < 0) {
    return -1;
  }
  if ((parsed_bytes = parse_suback_msg_id(data + parsed_bytes, data_len, &parsed_bytes, msg_id)) < 0) {
    return -1;
  }
  if ((parsed_bytes = parse_suback_return_code_byte(data + parsed_bytes, data_len, &parsed_bytes, return_code)) < 0) {
    return -1;
  }
  return parsed_bytes;
}
int32_t parse_suback_header(ParsedMqttSnHeader *h, const uint8_t *data, uint16_t data_len, int32_t *parsed_bytes) {
  ...
  if (h->length != MQTT_SN_MESSAGE_SUBACK_LENGTH) {
    // invalid message length for suback
    return -1;
  }
  return *parsed_bytes;
}
...
int32_t parse_suback_return_code_byte(const uint8_t *src_pos,
                                      uint16_t src_len,
                                      int32_t *parsed_bytes,
                                      MQTT_SN_RETURN_CODE *return_code) {
