...
int32_t parse_regack_bytes(uint16_t *topic_id,
  ...
  if ((parsed_bytes = parse_regack_header(&h, data, data_len, &parsed_bytes) < 0)) {
    return -1;
  }
  if ((parsed_bytes = parse_regack_topic_id(data + parsed_bytes, data_len, &parsed_bytes, topic_id)) < 0) {
    return -1;
  }
  if ((parsed_bytes = parse_regack_msg_id(data + parsed_bytes, data_len, &parsed_bytes, msg_id)) < 0) {
    return -1;
  }
  if ((parsed_bytes = parse_regack_return_code_byte(data + parsed_bytes, data_len, &parsed_bytes, return_code)) < 0) {
    return -1;
  }
  return parsed_bytes;
}
