...
static int32_t parse_unsubscribe_header(ParsedMqttSnHeader *h,
                                        const uint8_t *data,
                                        uint16_t data_len,
                                        int32_t *parsed_bytes);
static int32_t parse_unsubscribe_flags(const uint8_t *src_pos,
                                       uint16_t src_length,
                                       int32_t *parsed_bytes,
                                       uint8_t *dup,
                                       int8_t *qos,
                                       ...
                                       uint16_t length);
static int32_t parse_unsubscribe_msg_id(const uint8_t *src_pos,
                                        uint16_t src_length,
                                        int32_t *parsed_bytes,
                                        uint16_t *msg_id);
static int32_t parse_unsubscribe_predefined_topic(const uint8_t *src_pos,
                                                  uint16_t src_len,
                                                  int32_t *parsed_bytes,
                                                  uint16_t *topic_id);
static int32_t parse_unsubscribe_short_topic_name(const uint8_t *src_pos,
                                                  uint16_t src_len,
                                                  int32_t *parsed_bytes,
                                                  uint16_t *topic_id);
static int32_t parse_unsubscribe_topic_name(const uint8_t *src_pos,
                                            uint16_t src_len,
                                            int32_t *parse_bytes,
                                            char *topic_name,
...
int32_t parse_unsubscribe_byte(uint8_t *dup,
  ...
  if ((parsed_bytes = parse_unsubscribe_flags(data + parsed_bytes,
  ...
  if ((*topic_id_type) == MQTT_SN_FLAG_TOPIC_ID_TYPE_PREDEFINED_TOPIC_ID) {
    if ((parsed_bytes = parse_unsubscribe_predefined_topic(data + parsed_bytes, data_len, &parsed_bytes, topic_id))
        < 0) {
      return -1;
    }
  } else if ((*topic_id_type) == MQTT_SN_FLAG_TOPIC_ID_TYPE_SHORT_TOPIC_NAME) {
    if ((parsed_bytes = parse_unsubscribe_short_topic_name(data + parsed_bytes, data_len, &parsed_bytes, topic_id))
        < 0) {
      return -1;
    }
  } else if ((*topic_id_type) == MQTT_SN_FLAG_TOPIC_ID_TYPE_TOPIC_NAME) {
    if ((parsed_bytes = parse_unsubscribe_topic_name(data + parsed_bytes,
                                                     data_len,
                                                     &parsed_bytes,
                                                     topic_name,
                                                     topic_name_length,
...
int32_t parse_unsubscribe_flags(const uint8_t *src_pos,
                                uint16_t src_length,
                                int32_t *parsed_bytes,
                                uint8_t *dup,
                                int8_t *qos,
...
int32_t parse_unsubscribe_predefined_topic(const uint8_t *src_pos,
                                           uint16_t src_len,
                                           int32_t *parsed_bytes,
                                           uint16_t *topic_id) {
...
int32_t parse_unsubscribe_topic_name(const uint8_t *src_pos,
                                     uint16_t src_len,
                                     int32_t *parse_bytes,
                                     char *topic_name,
                                     uint16_t *topic_name_length,
...
int32_t parse_unsubscribe_short_topic_name(const uint8_t *src_pos,
                                           uint16_t src_len,
                                           int32_t *parsed_bytes,
                                           uint16_t *topic_id) {
  return parse_mqtt_sn_uint16_byte(src_pos, src_len, parsed_bytes, topic_id);
