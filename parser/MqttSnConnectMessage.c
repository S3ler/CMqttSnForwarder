...
static int32_t parse_connect_header(ParsedMqttSnHeader *h, const uint8_t *data, uint16_t data_len, int32_t *parsed_bytes);
static int32_t parse_connect_flags(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint8_t *will, uint8_t *clean_session);
static int32_t parse_connect_protocol_id(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint8_t *protocol_id);
static int32_t parse_connect_duration(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint16_t *duration);
static int32_t parse_connect_client_id(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, char *client_id,
                                       uint16_t *client_id_length, uint8_t client_id_max_length);
int32_t generate_connect_header(uint8_t *dst, uint16_t dst_len, const char *client_id, uint16_t client_id_length, int32_t *used_bytes);
...
int32_t generate_client_id(uint8_t *dst, uint16_t dst_len, const char *client_id, uint16_t client_id_length, int32_t *used_bytes);
...
int32_t parse_connect(ParsedMqttSnConnect *mqtt_sn_connect, const uint8_t *data, uint16_t data_len) {
    return parse_connect_message_byte(&mqtt_sn_connect->will, &mqtt_sn_connect->clean_session, &mqtt_sn_connect->protocol_id,
                                      &mqtt_sn_connect->duration, mqtt_sn_connect->client_id, &mqtt_sn_connect->client_id_length,
                                      sizeof(mqtt_sn_connect->client_id), data, data_len);
}
int32_t parse_connect_message_byte(uint8_t *will, uint8_t *clean_session, uint8_t *protocol_id, uint16_t *duration, char *client_id,
                                   uint16_t *client_id_length, uint8_t client_id_max_length, const uint8_t *data, uint16_t data_len) {
    int32_t            parsed_bytes = 0;
    ParsedMqttSnHeader h            = { 0 };
    if ((parsed_bytes = parse_connect_header(&h, data, data_len, &parsed_bytes)) < 0) {
        return -1;
    }
    if ((parsed_bytes = parse_connect_flags(data + parsed_bytes, data_len, &parsed_bytes, will, clean_session)) < 0) {
        return -1;
    }
    ...
    if ((parsed_bytes =
           parse_connect_client_id(data + parsed_bytes, data_len, &parsed_bytes, client_id, client_id_length, client_id_max_length)) < 0) {
        return -1;
    }
    return parsed_bytes;
}
int32_t generate_connect(uint8_t *dst, uint16_t dst_len, uint8_t will, uint8_t clean_session, uint8_t protocol_id, uint16_t duration,
    ...
    if (generate_connect_header(dst, dst_len, client_id, client_id_length, &used_bytes) < 0) {
        return -1;
    }
    if (generate_flags(dst + used_bytes, dst_len, MQTT_SN_FLAG_DUP_FIRST_TIME, MQTT_SN_FLAG_QOS_0, MQTT_SN_FLAG_RETAIN_FALSE, will,
                       clean_session, MQTT_SN_FLAG_TOPIC_ID_TYPE_TOPIC_NAME, &used_bytes) < 0) {
        return -1;
    }
    ...
    if (generate_client_id(dst + used_bytes, dst_len, client_id, client_id_length, &used_bytes) < 0) {
        return -1;
    }
    return used_bytes;
}
...
int32_t get_client_id_length(const char *client_id) {
    ...
    if (client_id_len == MQTT_SN_MESSAGE_CONNECT_CLIENTID_MAX_LENGTH && client_id[client_id_len] != '\0') {
        return -1;
    }
...
int32_t generate_connect_header(uint8_t *dst, uint16_t dst_len, const char *client_id, uint16_t client_id_length, int32_t *used_bytes) {
    ...
    if (client_id_length > MQTT_SN_MESSAGE_CONNECT_CLIENTID_MAX_LENGTH) {
        return -1;
    }
    if (client_id_length == MQTT_SN_MESSAGE_CONNECT_CLIENTID_MAX_LENGTH && client_id[client_id_length] != '\0') {
        return -1;
    }
    uint16_t total_length = MQTT_SN_MESSAGE_CONNECT_WO_CLIENTID_LENGTH + client_id_length;
...
int32_t parse_connect_flags(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint8_t *will, uint8_t *clean_session) {
    return parse_mqtt_sn_flags(src_pos, src_len, parsed_bytes, NULL, NULL, NULL, will, clean_session, NULL);
}
int32_t parse_connect_protocol_id(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, uint8_t *protocol_id) {
...
int32_t parse_connect_client_id(const uint8_t *src_pos, uint16_t src_len, int32_t *parsed_bytes, char *client_id,
                                uint16_t *client_id_length, uint8_t client_id_max_length) {
