...
int32_t init_discovery_handler(MqttSnGatewayDiscoveryHandler *handler, MqttSnClientNetworkInterface *clientNetwork,
                               MqttSnFixedSizeRingBuffer *clientNetworkSendBuffer, db_handler *db_handler_,
                               gateway_advertise_config *advertise_config, MqttSnLogger *logger) {
...
int32_t discovery_handler_parse_and_handle_advertise(MqttSnGatewayDiscoveryHandler *handler, MqttSnMessageData *msg, int32_t parsed_bytes,
    ...
    if (mqtt_sn_discovered_gateway_update(handler->db_handler_, mqtt_sn_advertise.gwId, mqtt_sn_advertise.duration,
                                          &forwarders->wireless_node_id, msg->signal_strength, msg->create_time_s,
                                          forwarders->forwarder_addresses,
                                          forwarders->forwarder_len) != DB_ENTRY_MQTT_SN_GATEWAY_DISCOVERED_RESULT_SUCCESS) {
#if WITH_LOGGING
        log_fatal_discovery_db_error(handler->logger, __FILE__, __func__, __LINE__, get_gateway_discovered_result(handler->db_handler_),
...
int32_t discovery_handler_parse_and_handle_search_gw(MqttSnGatewayDiscoveryHandler *handler, MqttSnMessageData *msg, int32_t parsed_bytes,
                                                     MqttSnGatewayForwarder *forwarders) {
    ...
    int32_t gen_rc           = generate_gwinfo_gateway_message(toSend.data, sizeof(toSend.data), handler->advertise_config->gateway_id);
    if (gen_rc < 0) {
#if WITH_LOGGING
...
int32_t discovery_handler_parse_and_handle_gwinfo(MqttSnGatewayDiscoveryHandler *handler, MqttSnMessageData *msg, int32_t parsed_bytes,
                                                  MqttSnGatewayForwarder *forwarders) {
    ...
    if (mqtt_sn_discovered_gateway_update_address(handler->db_handler_, mqtt_sn_gw_info.gwId, &forwarders->wireless_node_id,
                                                  msg->create_time_s, msg->signal_strength, forwarders->forwarder_addresses,
                                                  forwarders->forwarder_len) != DB_ENTRY_MQTT_SN_GATEWAY_DISCOVERED_RESULT_SUCCESS) {
#if WITH_LOGGING
        log_fatal_discovery_db_error(handler->logger, __FILE__, __func__, __LINE__, get_gateway_discovered_result(handler->db_handler_),
                                     get_gateway_discovered_transaction_result(handler->db_handler_));
...
int32_t discovery_handler_check_standy_by_monitoring(MqttSnGatewayDiscoveryHandler *handler, MQTT_SN_GATEWAY_STATUS *gateway_status) {
    ...
    if ((*gateway_status) == MQTT_SN_GATEWAY_STATUS_STAND_BY &&
        get_gateway_status(handler->db_handler_, handler->advertise_config->gateway_id) != DB_ENTRY_MQTT_SN_GATEWAY_DISCOVERED_ACTIVE) {
