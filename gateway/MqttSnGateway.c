...
uint16_t gateway_add_forwarder_encapsulation_frames(MqttSnMessageData *msg, int32_t gen_bytes, MqttSnGatewayForwarder *forwarders,
                                                    const MqttSnLogger *logger);
int32_t  MqttSnGatewayHandleClientMessageData(MqttSnGateway *mqttSnGateway, MqttSnMessageData *clientMessageData);
int32_t  parse_and_handle_encapsulated_messages(MqttSnGateway *gateway, MqttSnMessageData *msg, int32_t parsed_bytes,
                                                MqttSnGatewayForwarder *forwarders);
int32_t  parse_and_handle_message(MqttSnGateway *mqttSnGateway, MqttSnMessageData *clientMessageData, int32_t parsed_bytes,
                                  MqttSnGatewayForwarder *forwarders);
int32_t  MqttSnGatewayInitialize(MqttSnGateway *mqttSnGateway, const MqttSnLogger *logger, MqttClient *mqttClient,
                                 void *clientNetworkContext, const gateway_advertise_config *gacfg,
    ...
    if (gccccfg) {
        if (init_client_connection_handler(&mqttSnGateway->client_con_handler_, &mqttSnGateway->clientNetwork,
                                           &mqttSnGateway->clientNetworkSendBuffer, &mqttSnGateway->db_handler_, gccccfg,
    ...
    if (gacfg) {
        ...
        if (init_discovery_handler(&mqttSnGateway->discovery_handler, &mqttSnGateway->clientNetwork,
                                   &mqttSnGateway->clientNetworkSendBuffer, &mqttSnGateway->db_handler_,
                                   &mqttSnGateway->advertisement_config, &mqttSnGateway->logger) < 0) {
            MqttSnGatewayDeinitialize(mqttSnGateway);
            return -1;
        }
    }
...
int32_t MqttSnGatewayLoop(MqttSnGateway *mqttSnGateway) {
    if (ClientNetworkReceive(&mqttSnGateway->clientNetwork, &mqttSnGateway->clientNetworkReceiveBuffer,
                             mqttSnGateway->clientNetworkReceiveTimeout, mqttSnGateway->clientNetworkContext) < 0) {
        ClientNetworkDisconnect(&mqttSnGateway->clientNetwork, mqttSnGateway->clientNetworkContext);
    }
#ifdef Arduino_h
    yield();
    ...
    if (ClientNetworkSend(&mqttSnGateway->clientNetwork, &mqttSnGateway->clientNetworkSendBuffer, mqttSnGateway->clientNetworkSendTimeout,
                          mqttSnGateway->clientNetworkContext) < 0) {
        ClientNetworkDisconnect(&mqttSnGateway->clientNetwork, mqttSnGateway->clientNetworkContext);
    }

    if (mqttSnGateway->client_con_handler_.client_connection_timeout_check_enabled) {
        if (client_connection_handler_check_timeouts(&mqttSnGateway->client_con_handler_) < 0) {
            return -1;
        }
    }
...
int32_t MqttSnGatewayHandleClientMessageData(MqttSnGateway *mqttSnGateway, MqttSnMessageData *clientMessageData) {
    ...
    if (PlatformCompatibilityGetTimestampMs(&clientMessageData->create_time_s) < 0) {
        return -1;
    }
    // TODO add receive time to message
...
int32_t parse_and_handle_stand_by_messages(MqttSnGateway *gateway, const ParsedMqttSnHeader *header, MqttSnMessageData *msg,
                                           int32_t parsed_bytes, MqttSnGatewayForwarder *forwarders) {
    switch (header->msg_type) {
    case ADVERTISE:
        return discovery_handler_parse_and_handle_advertise(&gateway->discovery_handler, msg, parsed_bytes, forwarders);
    case SEARCHGW:
        return discovery_handler_parse_and_handle_search_gw(&gateway->discovery_handler, msg, parsed_bytes, forwarders);
    case GWINFO:
        return discovery_handler_parse_and_handle_gwinfo(&gateway->discovery_handler, msg, parsed_bytes, forwarders);
    case ENCAPSULATED_MESSAGE:
        return parse_and_handle_encapsulated_messages(gateway, msg, parsed_bytes, forwarders);
    default:
        return 1;
    }
    return 0;
}
int32_t parse_and_handle_active_messages(MqttSnGateway *gateway, const ParsedMqttSnHeader *header, MqttSnMessageData *msg,
                                         int32_t parsed_bytes, MqttSnGatewayForwarder *forwarders) {
    switch (header->msg_type) {
    case ADVERTISE:
        return discovery_handler_parse_and_handle_advertise(&gateway->discovery_handler, msg, parsed_bytes, forwarders);
    ...
    case GWINFO:
        return discovery_handler_parse_and_handle_gwinfo(&gateway->discovery_handler, msg, parsed_bytes, forwarders);
    case CONNECT:
        return client_connection_handler_parse_and_handle_connect(&gateway->client_con_handler_, msg, parsed_bytes, forwarders);
    case CONNACK:
        return 1;
    case WILLTOPICREQ:
        return 1;
    ...
    case UNSUBACK:
        return 1;
    case PINGREQ:
        return client_connection_handler_parse_and_handle_ping_req(&gateway->client_con_handler_, msg, parsed_bytes, forwarders);
    case PINGRESP:
        return client_connection_handler_parse_and_handle_ping_resp(&gateway->client_con_handler_, msg, parsed_bytes, forwarders);
    case DISCONNECT:
        return client_connection_handler_parse_and_handle_disconnect(&gateway->client_con_handler_, msg, parsed_bytes, forwarders);
        // TODO parse_searchgw(address, bytes);
        break;
    case WILLTOPICUPD:
        // TODO parse_searchgw(address, bytes);
...
int32_t parse_and_handle_message(MqttSnGateway *mqttSnGateway, MqttSnMessageData *clientMessageData, int32_t parsed_bytes,
    ...
    if (parse_header(&header, ANY_MESSAGE_TYPE, clientMessageData->data + parsed_bytes, clientMessageData->data_length - parsed_bytes,
                     &parsed_header_bytes) < 0) {
#ifdef WITH_LOGGING
        log_malformed_message(&mqttSnGateway->logger, clientMessageData);
#endif
        return 0;
    }
    ...
    case MQTT_SN_GATEWAY_STATUS_STAND_BY: {
        int32_t handle_rc = parse_and_handle_stand_by_messages(mqttSnGateway, &header, clientMessageData, parsed_bytes, forwarders);
        if (handle_rc <= 0) {
            return handle_rc;
        }
        break;
    ...
    case MQTT_SN_GATEWAY_STATUS_ACTIVE: {
        int32_t handle_rc = parse_and_handle_active_messages(mqttSnGateway, &header, clientMessageData, parsed_bytes, forwarders);
        if (handle_rc <= 0) {
            return handle_rc;
        }
        break;
...
#ifdef WITH_LOGGING
    log_gateway_unhandled_message(&mqttSnGateway->logger, mqttSnGateway->gateway_status_, &forwarders->wireless_node_id,
                                  &clientMessageData->to, clientMessageData->signal_strength, clientMessageData->data + parsed_bytes,
...
int32_t parse_and_handle_encapsulated_messages(MqttSnGateway *gateway, MqttSnMessageData *msg, int32_t parsed_bytes,
    ...
    int32_t parse_rc =
      parse_multiple_forwarder_encapsulation_headers_byte(forwarders->forwarder_radiuses, forwarders->forwarder_addresses,
                                                          &forwarders->forwarder_len, forwarders->forwarder_max_len, &msg->from,
                                                          &forwarders->wireless_node_id, msg->data, msg->data_length);
    if (parse_rc < 0) {
        return 0;
    }
