...
int main() {
    ...
    for (auto &client : mqttSnNetworkBuilder->getMqttSnClients()) {
        ...
        std::shared_ptr<MqttSnClientActionSequence> action_sequence_connect = client->generateActionSequence(1);
        std::string          registered_first = "registered/topic";
        std::vector<uint8_t> payload          = client->getPayload(1);
        auto                 publish_action   = client->getPublishAction(registered_first, 1, false, payload);
        client->addAction(publish_action);
        std::shared_ptr<MqttSnClientActionSequence> action_sequence_publish = client->generateActionSequence(rounds);
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
...
void configureMqttClients(int32_t rounds, const vector<std::shared_ptr<MqttClientTestContainerInterface>> &clients) {
    ...
    for (auto &client : clients) {
        auto delay_action = client->getDelayAction(60);
