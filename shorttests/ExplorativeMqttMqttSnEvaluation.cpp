...
int main() {
    ...
    int32_t mqttClientCount            = 500;
    int32_t mqttSnGatewayCount         = 0;
...
void configureMqttClients(int32_t rounds, const vector<std::shared_ptr<MqttClientTestContainerInterface>> &clients) {
    ...
    for (auto &client : clients) {
        std::vector<uint8_t> payload        = client->getPayload(16384); // 16 kB payload
...
/*
    ...
    for (auto &client : clients) {
        std::vector<uint8_t> payload        = client->getPayload(16384); // 16 kB payload
        auto                 publish_action = client->getPublishAction(client->configuration.clientId, payload, 1, false);
