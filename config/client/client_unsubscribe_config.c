...
int32_t parse_client_subscribe_config(const MqttSnLogger *logger, char *url, single_client_unsubscribe_config *cfg);
...
int32_t is_client_unsubscribe_config_command(char *arg, int *i) {
  if (!strcmp(arg, "-ui") || !strcmp(arg, "--unsubscribe_topic_id")) {
    (*i)++;
    return 1;
  } else if (!strcmp(arg, "-up") || !strcmp(arg, "--unsubscribe_predefined_topic_id")) {
    (*i)++;
    return 1;
  } else if (!strcmp(arg, "-un") || !strcmp(arg, "--unsubscribe_topic_name")) {
    (*i)++;
    return 1;
...
int32_t client_unsubscribe_config_process_args(client_unsubscribe_config *cfg,
  ...
  for (int i = 0; i < argc; i++) {
    if (!strcmp(argv[i], "-ui") || !strcmp(argv[i], "--unsubscribe_topic_id")) {
      if (i == argc - 1) {
        ...
        if (cfg->unsubscribe_list_len < cfg->unsubscribe_list_max_len) {
          cfg->unsubscribe_list[cfg->unsubscribe_list_len].topic_id_type =
              CLIENT_MQTT_SN_UNSUBSCRIPTION_TOPIC_ID_TYPE_SHORT;
      ...
      i++;
    } else if (!strcmp(argv[i], "-up") || !strcmp(argv[i], "--unsubscribe_predefined_topic_id")) {
      if (i == argc - 1) {
        ...
        if (cfg->unsubscribe_list_len < cfg->unsubscribe_list_max_len) {
          cfg->unsubscribe_list[cfg->unsubscribe_list_len].topic_id_type =
              CLIENT_MQTT_SN_UNSUBSCRIPTION_TOPIC_ID_TYPE_PREDEFINED;
          if (parse_topic_id(logger, argv[i + 1], &cfg->unsubscribe_list[cfg->unsubscribe_list_len].topic_id)
              == MQTT_SN_PARSE_CONFIG_FAILURE) {
            return MQTT_SN_PARSE_CONFIG_FAILURE;
          }
      ...
      i++;
    } else if (!strcmp(argv[i], "-un") || !strcmp(argv[i], "--unsubscribe_topic_name")) {
      if (i == argc - 1) {
        ...
        if (cfg->unsubscribe_list_len < cfg->unsubscribe_list_max_len) {
          cfg->unsubscribe_list[cfg->unsubscribe_list_len].topic_id_type =
              CLIENT_MQTT_SN_UNSUBSCRIPTION_TOPIC_ID_TYPE_TOPICNAME;
...
int32_t parse_client_subscribe_config(const MqttSnLogger *logger, char *url, single_client_unsubscribe_config *cfg) {
  ...
  if (topic_id_type_str) {
    if (!strcmp(topic_id_type_str, "topicname")) {
      ...
      cfg->topic_id_type = CLIENT_MQTT_SN_UNSUBSCRIPTION_TOPIC_ID_TYPE_SHORT;
    } else {
      return MQTT_SN_PARSE_CONFIG_FAILURE;
    }
  } else {
...
void client_unsubscribe_config_print_usage_short(const MqttSnLogger *logger, const char *indent) {
  ...
  log_str(logger, PSTR("{[-ui --unsubscribe_topic_id] [-up ---unsubscribe_predefined_topic_id] "
                       "[-un --unsubscribe_topic_name] [-uc --unsubscribe_clear]| [-uL --unsubscribe_URL]}\n"));
}
void client_unsubscribe_config_print_usage_long(const MqttSnLogger *logger) {
  log_str(logger, PSTR(" -ui : unsubscribe topic id.\n"));
  log_str(logger, PSTR(" -up : unsubscribe predefined topic id.\n"));
  log_str(logger, PSTR(" -up : unsubscribe clear can be true or false. Deletes the saved unsubscribe list.\n"));
