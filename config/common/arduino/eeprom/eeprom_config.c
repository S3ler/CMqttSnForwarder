...
int process_eeprom_force_set_str(EEPROM_cfg *ecfg, const MqttSnLogger *logger, char *ecfg_line, size_t ecfg_line_len) {
    ...
    const char *efl       = NULL;
    if (ecfg_line_len > strlen(efl_short) && strlen(ecfg_line) > strlen(efl_short) && StartsWith(efl_short, ecfg_line)) {
        efl = efl_short;
    } else if (ecfg_line_len > strlen(efl_long) && strlen(ecfg_line) > strlen(efl_long) && StartsWith(efl_long, ecfg_line)) {
        efl = efl_long;
    } else {
        return -1;
    }
