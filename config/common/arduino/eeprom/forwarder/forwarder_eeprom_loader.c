...
int eeprom_load_forwarder_config(EEPROM_cfg *ecfg, void *fcfg, const char *executable_name, const MqttSnLogger *logger,
    ...
    if (forwarder_config_file_process_command_callback == NULL && config_print_usage == NULL) {
        return 0;
    }
    int32_t fcfg_rc = process_config_file_line(logger, ecfg->fcfg_line, strlen(ecfg->fcfg_line) + 1, executable_name, fcfg,
                                               forwarder_config_file_process_command_callback);

    if (fcfg_rc == MQTT_SN_PARSE_CONFIG_HELP) {
        config_print_usage(logger);
...
int eeprom_save_forwarder_config_line(EEPROM_cfg *ecfg, void *fcfg, const char *executable_name, const MqttSnLogger *logger,
                                      char *fcfg_line, size_t fcfg_line_len,
                                      int32_t (*forwarder_config_file_process_command_callback)(void *cfg, const MqttSnLogger *logger,
                                                                                                int argc, char **argv),
                                      void (*config_print_usage)(const MqttSnLogger *logger)) {
    assert(logger != NULL);
    ...
    int32_t fcfg_rc = MQTT_SN_PARSE_CONFIG_SUCCESS;
    if (forwarder_config_file_process_command_callback && config_print_usage) {
        fcfg_rc =
          process_config_file_line(logger, line_copy, fcfg_line_len, executable_name, fcfg, forwarder_config_file_process_command_callback);
    }

    if (fcfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE) {
        print_eeprom_line_parse_error(logger, fcfg_line, fcfg_line_len);
    }
    if (fcfg_rc == MQTT_SN_PARSE_CONFIG_SUCCESS) {
    ...
    if (fcfg_rc != MQTT_SN_PARSE_CONFIG_SUCCESS && fcfg_rc != MQTT_SN_PARSE_CONFIG_HELP) {
        log_str(logger, PSTR("Use --help to see usage."));
        log_flush(logger);
    }
    return fcfg_rc;
}
