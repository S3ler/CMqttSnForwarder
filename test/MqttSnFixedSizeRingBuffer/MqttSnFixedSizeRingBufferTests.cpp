...
EXPECT_EQ(ringBuffer
.maxlen, CMQTTSNFORWARDER_MQTTSNFIXEDSIZERINGBUFFER_MAXLEN);
}
...
for (
uint32_t i = 0;
i<CMQTTSNFORWARDER_MQTTSNFIXEDSIZERINGBUFFER_MAXLEN;
i++) {
MqttSnMessageData zeroItem = {0};
EXPECT_EQ(memcmp(&ringBuffer.items[i], &zeroItem, sizeof(MqttSnMessageData)),
0);
}
}
...
MqttSnMessageDataGenerator::generateData(mqttSnMessageData
.data, mqttSnMessageData.data_length);

ASSERT_EQ(put(&ringBuffer, &mqttSnMessageData),
0);
...
i<CMQTTSNFORWARDER_MQTTSNFIXEDSIZERINGBUFFER_MAXLEN;
i++) {
ASSERT_EQ(pop(&ringBuffer, &popMqttSnMessageData),
0);
}
...
EXPECT_EQ(put(&ringBuffer, &mqttSnMessageData),
0);
}
...
for (
uint32_t i = 0;
i<CMQTTSNFORWARDER_MQTTSNFIXEDSIZERINGBUFFER_MAXLEN - 1; i++) {
ASSERT_EQ(put(&ringBuffer, &mqttSnMessageData),
0);
}
EXPECT_EQ(put(&ringBuffer, &mqttSnMessageData),
...
MqttSnMessageData popMqttSnMessageData = {0};
EXPECT_EQ(pop(&ringBuffer, &popMqttSnMessageData),
0);
...
i < 1; ++i) {
MqttSnMessageData mqttSnMessageData = {0};
MqttSnMessageDataGenerator::generateDeviceAddressFromUint32(&mqttSnMessageData
.address, i);
mqttSnMessageData.
data_length = CMQTTSNFORWARDER_MAXIMUM_MESSAGE_LENGTH;
MqttSnMessageDataGenerator::generateData(mqttSnMessageData
.data, mqttSnMessageData.data_length);
mqttSnMessageDatas.
push_back(mqttSnMessageData);
}

for (
auto mqttSnMessageData
: mqttSnMessageDatas) {
ASSERT_EQ(put(&ringBuffer, &mqttSnMessageData),
0);
}
