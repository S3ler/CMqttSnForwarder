...
TEST_P(MqttSnClientNetworkInterfaceMessageDefragmentationTests, DefragmentationTests) {
  ...
  std::vector<std::vector<std::vector<uint8_t>>> vecs;
  for (const auto &mockClient : mockClients) {
    ...
    for (auto data : mockClientMqttSnMessageDatas) {
      v.insert(v.end(),
  ...
  ASSERT_EQ(vecs.size(), mockClients.size()) << "should never happen";

  for (uint64_t k = 0; k < global_size; ++k) {
    for (uint64_t c = 0; c < mockClients.size(); ++c) {
