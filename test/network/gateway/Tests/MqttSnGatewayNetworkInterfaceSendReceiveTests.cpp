...
ASSERT_TRUE(gatewayNetworkForwarderLooper
.
pauseLoop()
);
...
messageCounter<toTestMessageCount;
++messageCounter) {
if (useIdentifier) {
*mockGateway->
getGatewayDeviceAddress() = getDeviceAddressFromNetworkContext(mockGateway->getIdentifier(),
...
ComparableGatewayMqttSnMessageData expectedData(toTestMessageLength,
                                                ...
                                                useIdentifier);
expectedMockGatewayMqttSnMessageDatas.
push_back(expectedData);
ComparableGatewayMqttSnMessageData toSendData(toTestMessageLength,
                                              mockGateway->getGatewayDeviceAddress(),
                                              mockGateway->getIdentifier(),
                                              useIdentifier);
forwarderMqttSnMessageDataBuffer.
push_back(toSendData);
}
...
->
getReceiver(), receive_any_message(_, _, _)
)
.
...
.
Times(expectedMockGatewayMqttSnMessageDatas
.
size()
)
.
