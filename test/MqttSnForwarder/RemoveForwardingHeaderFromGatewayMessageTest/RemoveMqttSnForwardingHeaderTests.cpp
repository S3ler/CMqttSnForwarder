...
gatewayMessageData.
data_length = MQTT_SN_ENCAPSULATED_MESSAGE_HEADER_LENGTH(false) + MQTT_SN_MINIMAL_MESSAGE_LENGTH;
gatewayMessageData.data[0] = gatewayMessageData.
data_length;
gatewayMessageData.data[1] =
PINGRESP;
EXPECT_EQ(RemoveMqttSnForwardingHeader(&gatewayMessageData, &clientMessageData),
-1);
}
...
gatewayMessageData.
data_length = CMQTTSNFORWARDER_MAXIMUM_MESSAGE_LENGTH + 1;
EXPECT_EQ(RemoveMqttSnForwardingHeader(&gatewayMessageData, &clientMessageData),
-1);
}
...
0, sizeof(MqttSnMessageData));
gatewayMessageData.
data_length = MQTT_SN_ENCAPSULATED_MESSAGE_HEADER_LENGTH(false);
memset(&gatewayMessageData
.data, 0, CMQTTSNFORWARDER_MAXIMUM_MESSAGE_LENGTH);
EXPECT_EQ(RemoveMqttSnForwardingHeader(&gatewayMessageData, &clientMessageData),
-1);
...
data_length = MQTT_SN_ENCAPSULATED_MESSAGE_HEADER_LENGTH(false) + MQTT_SN_MINIMAL_MESSAGE_LENGTH;
gatewayMessageData.data[0] = MQTT_SN_ENCAPSULATED_MESSAGE_HEADER_LENGTH(false) +
MQTT_SN_MINIMAL_MESSAGE_LENGTH;
gatewayMessageData.data[1] =
ENCAPSULATED_MESSAGE;
EXPECT_EQ(RemoveMqttSnForwardingHeader(&gatewayMessageData, &clientMessageData),
0);
