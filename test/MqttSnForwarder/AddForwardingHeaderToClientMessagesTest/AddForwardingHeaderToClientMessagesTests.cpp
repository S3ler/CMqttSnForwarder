...
TEST_F(AddForwardingHeaderToClientMessagesTests,
  ...
  ASSERT_EQ(AddForwardingHeaderToClientMessages(&mqttSnForwarder, &clientMessageData, &gatewayMessageData), 0);
  ComparableMqttSnMessageData actualComparableMqttSnData(gatewayMessageData);

  MqttSnMessageData mqttSnMessageDataZeroed = {0};
  mqttSnMessageDataZeroed.data_length = 3 + sizeof(device_address);
  mqttSnMessageDataZeroed.data[0] = mqttSnMessageDataZeroed.data_length;
  mqttSnMessageDataZeroed.data[1] = 0xFE;
  ComparableMqttSnMessageData expectedComparableMqttSnData(mqttSnMessageDataZeroed);

  EXPECT_EQ(actualComparableMqttSnData.data_length, expectedComparableMqttSnData.data_length);
  EXPECT_EQ(actualComparableMqttSnData.address, expectedComparableMqttSnData.address);
  EXPECT_EQ(actualComparableMqttSnData.data, expectedComparableMqttSnData.data);
}
