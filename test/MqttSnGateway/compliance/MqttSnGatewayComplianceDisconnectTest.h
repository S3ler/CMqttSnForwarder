...
class MqttSnGatewayComplianceDisconnectTest : public ::testing::TestWithParam<std::tuple<MqttBrokerTestType, MqttSnGatewayTestType>> {
 public:
...
struct PrintToStringMqttSnGatewayComplianceDisconnectTestValueName {
  ...
  std::string operator()(const ::testing::TestParamInfo<ParamType> &info) const {
    std::string result = std::string(getTextFromEnum(std::get<0>(info.param))) + "_" + std::string(getTextFromEnum(std::get<1>(info.param)));
    return result;
  }
};

INSTANTIATE_TEST_SUITE_P(MqttSnGatewayComplianceDisconnectCompareTest, MqttSnGatewayComplianceDisconnectTest,
                         testing::Combine(testing::Values(MqttBrokerTestType::MOSQUITTO_DOCKER), testing::Values(MqttSnGatewayTestType::ARSMB, MqttSnGatewayTestType::PAHO)),
