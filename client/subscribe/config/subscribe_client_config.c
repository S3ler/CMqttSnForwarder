...
#include "subscribe_client_config.h"
...
int32_t subscribe_client_config_process_line(subscribe_client_config *cfg, const MqttSnLogger *logger, int argc, char **argv) {
    ...
    if (config_file_index) {
        ...
        if (process_config_file(config_file_path, logger, cfg, subscribe_client_config_file_process_command_callback, argv[0]) ==
    ...
    int32_t cfg_rc = subscribe_client_config_process_args(cfg, logger, argc, argv);
    ...
    if (msgfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || gnfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || mslcfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE ||
        ...
        crcfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || cscfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || cfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE ||
        scfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE) {
        return MQTT_SN_PARSE_CONFIG_FAILURE;
    }
    if (client_file_rc + msgfg_rc + gnfg_rc + mslcfg_rc + cnpcfg_rc + csrcfg_rc + cfmsgcfg_rc + cccfg_rc + crcfg_rc + cscfg_rc + cfg_rc +
          scfg_rc !=
        (argc - 1)) {
        // there is an unknown option
        for (int i = 1; i < argc; i++) {
            if (!is_subscribe_client_config_command(argv[i], &i) & !is_gateway_network_config_command(argv[i], &i) &
                ...
                !is_client_subscribe_config_command(argv[i], &i) & !is_mqtt_sn_gateway_config_command(argv[i], &i) &
...
int32_t mqtt_sn_cmd_buffer_subscribe_client_config_simulate(const subscribe_client_config *cfg, const MqttSnLogger *logger, int argc,
                                                            char **argv) {
    subscribe_client_config cfg_cp = (*cfg);

    int32_t rc = subscribe_client_config_process_line(&cfg_cp, logger, argc, argv);
    ...
    if (subscribe_client_config_copy_to_buffer(&cfg_cp) < 0) {
        return MQTT_SN_PARSE_CONFIG_FAILURE;
    }
    return 0;
}
...
void subscribe_client_config_print_usage(const MqttSnLogger *logger) {
    log_str(logger, PSTR("msclient_subscribe"));
    ...
    subscribe_client_config_print_usage_short(logger);
    log_flush(logger);
    log_flush(logger);
    subscribe_client_config_print_usage_long(logger);
...
int32_t subscribe_client_config_file_process_command_callback(void *cfg, const MqttSnLogger *logger, int argc, char **argv) {
    subscribe_client_config *pccfg = cfg;
    return subscribe_client_config_process_line(pccfg, logger, argc, argv);
}
...
int32_t parse_initial_subscribe_client_config(subscribe_client_config *cfg, const MqttSnLogger *cfg_logger, int argc, char **argv) {
    int rc = subscribe_client_config_process_line(cfg, cfg_logger, argc, argv);
    if (rc) {
        if (rc == MQTT_SN_PARSE_CONFIG_HELP) {
            subscribe_client_config_print_usage(cfg_logger);
        } else {
            subscribe_client_config_print_see_usage(cfg_logger);
            subscribe_client_config_cleanup(cfg);
            return EXIT_FAILURE;
        }
        subscribe_client_config_cleanup(cfg);
        return MQTT_SN_PARSE_CONFIG_HELP;
    }
    if (subscribe_client_config_copy_to_buffer(cfg) < 0) {
        subscribe_client_config_print_not_copyable(cfg_logger);
        subscribe_client_config_cleanup(cfg);
        return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
