...
#include "interactive_client_starter.h"
...
int32_t send_single_mqtt_sn_m1_publish(MqttSnClient *interactive_client, const interactive_client_config *cfg);
static void sig_handler(int _) {
  (void) _;
  keep_running = 0;
}

void *interactive_client_thread_function(void *ptr) {
  MqttSnClient *interactive_client = ((PublishClient_cfg_ptr *) ptr)->c_ptr;
  while ((MqttSnClientLoop(interactive_client) >= 0 && interactive_client->connected) & keep_running) {}
...
int32_t start_interactive_client(const interactive_client_config *cfg,
  ...
  if (gatewayNetworkContext == NULL) {
    ...
    if (cfg->gncfg.gateway_network_plugin_path != NULL) {
      int rc = start_interactive_client_plugin(cfg, interactive_client, logger, gatewayNetworkContext);
      // TODO
    }
#endif
#ifdef WITH_LINUX_UDP_GATEWAY_NETWORK
    if (!strcmp(cfg->gncfg.gateway_network_protocol, "udp")) {
      int rc = start_interactive_client_udp(cfg, interactive_client, logger, gatewayNetworkContext);
    }
#endif
#ifdef WITH_LINUX_TCP_GATEWAY_NETWORK1
    if (!strcmp(cfg->gncfg.gateway_network_protocol, "tcp")) {
      int rc = start_interactive_client_tcp(cfg, interactive_client, logger, gatewayNetworkContext);
  ...
  interactive_client->gatewayNetworkSendTimeout = cfg->gncfg.gateway_network_send_timeout;
  interactive_client->gatewayNetworkReceiveTimeout = cfg->gncfg.gateway_network_receive_timeout;
#ifdef WITH_LOGGING
  print_program_started(logger, &cfg->msvcfg, cfg->executable_name);
#endif
  if (MqttSnClientInit(interactive_client, logger, gatewayNetworkContext) < 0) {
    MqttSnClientDeinit(interactive_client);
    return EXIT_FAILURE;
  }
...
#ifdef WITH_LOGGING
    print_program_terminated(&interactive_client->logger, &cfg->msvcfg, cfg->executable_name);
#endif
    return EXIT_SUCCESS;
  }
  */
  ...
  if (only_M1_publishes) {
    return send_single_mqtt_sn_m1_publish(interactive_client, cfg);
  }


  MQTT_SN_CLIENT_RETURN_CODE connect_rc;
  MqttSnClientTimeoutOffset(interactive_client, cfg->cccfg.connect_timeout_offset);
  if ((connect_rc = MqttSnClientDirectConnect(interactive_client,
                                              interactive_client->gatewayNetwork.mqtt_sn_gateway_address,
                                              cfg->cccfg.client_connect_timeout_ms,
                                              cfg->cccfg.clean_session,
                                              cfg->cccfg.client_id,
                                              cfg->cccfg.connect_duration)) != MQTT_SN_CLIENT_RETURN_SUCCESS) {

    log_mqtt_sn_client(&interactive_client->logger, connect_rc);
    // TODO log error here
#ifdef WITH_LOGGING
    print_program_terminated(&interactive_client->logger, &cfg->msvcfg, cfg->executable_name);
#endif
  ...
  pthread_t thread;
  PublishClient_cfg_ptr ptr = {.c_ptr = interactive_client};
  if (pthread_create(&thread, NULL, interactive_client_thread_function, &ptr)) {
...
int32_t send_single_mqtt_sn_m1_publish(MqttSnClient *interactive_client,
                                       const interactive_client_config *cfg) {// no connect needed
  for (uint32_t i = 0; i < cfg->cpcfg.publish_list_len; i++) {
    single_client_publish_config publish = cfg->cpcfg.publish_list[i];
    if (MqttSnClientPublishPredefinedM1(interactive_client, publish.topic_id, 0, publish.data, publish.data_length) < 0) {
      MqttSnClientDeinit(interactive_client);
      return EXIT_FAILURE;
    }
  }
...
int32_t start_interactive_client_plugin(const interactive_client_config *cfg,
  ...
  if (GatewayNetworkInitialize(&interactive_client->gatewayNetwork,
  ...
  return start_interactive_client(cfg, logger, interactive_client, gatewayNetworkContext);
}
#endif
#ifdef WITH_LINUX_UDP_GATEWAY_NETWORK
