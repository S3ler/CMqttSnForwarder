...
int32_t interactive_client_config_file_process_command_callback(void *cfg, const MqttSnLogger *logger, int argc, char *argv[]) {
    interactive_client_config_container *pccfg = (interactive_client_config_container *) cfg;
...
int32_t interactive_client_config_process_line(interactive_client_config_container *cfg, const MqttSnLogger *logger, int argc, char *argv[]) {
    ...
    if (config_file_index) {
        ...
        if (process_config_file(config_file_path, logger, cfg, interactive_client_config_file_process_command_callback, argv[0]) ==
    ...
    for (int i = 1; i < argc; i++) {
        if (!is_interactive_client_config_command(argv[i], &i) & !is_gateway_network_config_command(argv[i], &i) &
            ...
            !is_client_subscribe_config_command(argv[i], &i) & !is_client_unsubscribe_config_command(argv[i], &i) &
    ...
    */
    ...
    if (msgfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || gnfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || mslcfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE ||
        ...
        crcfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || cscfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || cuscfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE ||
        cpcfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || cfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE || scfg_rc == MQTT_SN_PARSE_CONFIG_FAILURE) {
        return MQTT_SN_PARSE_CONFIG_FAILURE;
    }
    if (client_file_rc + msgfg_rc + gnfg_rc + mslcfg_rc + cnpcfg_rc + csrcfg_rc + cfmsgcfg_rc + cccfg_rc + crcfg_rc + cscfg_rc + cuscfg_rc +
          cpcfg_rc + cfg_rc + scfg_rc !=
        (argc - 1)) {
...
int32_t mqtt_sn_cmd_buffer_interactive_client_simulate(interactive_client_config_container *cfg, const MqttSnLogger *logger, int argc,
    ...
    yield();
        int32_t rc = interactive_client_config_process_line(cfg, logger, argc, argv);
...
#else
    interactive_client_config_container cfg_cp = (*cfg);
    int32_t rc = interactive_client_config_process_line(&cfg_cp, logger, argc, argv);
    if (rc == MQTT_SN_PARSE_CONFIG_HELP) {
        return rc;
    }
    if (rc < 0) {
